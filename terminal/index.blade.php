...
<x-admin.wrapper>
        ...
        @can('terminal create')
        <x-admin.add-link href="{{ route('terminal.create') }}">
            {{ __('Add Terminal') }}
        </x-admin.add-link>
        @endcan
        ...
        <div class="py-2">
            <div class="min-w-full border-b border-gray-200 shadow overflow-x-auto">
                <x-admin.grid.search action="{{ route('terminal.index') }}" />
                <x-admin.grid.table>
                    <x-slot name="head">
                        <tr>
                            <x-admin.grid.th>
                            ...
                            @canany(['terminal edit', 'terminal delete'])
                            <x-admin.grid.th>
                                {{ __('Actions') }}
                            </x-admin.grid.th>
                            @endcanany
                        ...
                        <tr>
                            <x-admin.grid.td>
                                <div class="text-sm text-gray-900">
                                    <a href="{{route('terminal.show', $terminal->id)}}" class="no-underline hover:underline text-cyan-600"><img style="width: 100px" src="{{ asset($terminal->icon) }}" alt="" class="image"></a>
                                </div>
                            </x-admin.grid.td>
                            <x-admin.grid.td>
                                <div class="text-sm text-gray-900">
                                    <a href="{{route('terminal.show', $terminal->id)}}" class="no-underline hover:underline text-cyan-600">{{ $terminal->model }}</a>
                                </div>
                            </x-admin.grid.td>
                            <x-admin.grid.td>
                                <div class="text-sm text-gray-900">
                                    <a href="{{route('terminal.show', $terminal->id)}}" class="no-underline hover:underline text-cyan-600">{{ $terminal->submodel }}</a>
                                </div>
                            </x-admin.grid.td>
                            <x-admin.grid.td>
                                <div class="text-sm text-gray-900">
                                    <a href="{{route('terminal.show', $terminal->id)}}" class="no-underline hover:underline text-cyan-600">{{ $terminal->manufacturers->name }}</a>
                                </div>
                            </x-admin.grid.td>
                            <x-admin.grid.td>
                                <div class="text-sm text-gray-900">
                                    <a href="{{route('terminal.show', $terminal->id)}}" class="no-underline hover:underline text-cyan-600">{{ $terminal->year }}</a>
                                </div>
                            </x-admin.grid.td>
                            @canany(['terminal edit', 'terminal delete'])
                            <x-admin.grid.td>
                                <form action="{{ route('terminal.destroy', $terminal->id) }}" method="POST">
                                    <div class="flex">
                                        @can('terminal edit')
                                        <a href="{{route('terminal.edit', $terminal->id)}}"  class="btn btn-square btn-ghost">
...
<x-app-layout>
    ...
    <script type = "text/javascript">
      ...
      <?php
        ...
        $sql = "SELECT id, year, model, submodel, icon, manufacturer_id FROM terminals";
